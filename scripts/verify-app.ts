...
async function main(): Promise<void> {
	...
	await scenario("bg. an asset is generated by pointing, with no prompt box anywhere", async () => {
		// The claim this scenario exists to check is a negative one: at no point is
		// the user asked to describe what they want in words. Everything else here
		// is the ordinary end-to-end — questions, recipes, variants, a name, a real
		// file with real provenance — but the absence is the feature.
		await chrome.getByTestId("assets-generate").click()
		const panel = chrome.getByTestId("generate-asset")
		await panel.waitFor({ timeout: 15_000 })

		// Two questions, answered by clicking. A free-text input on either would be
		// the prompt box wearing a different hat.
		for (const [questionIndex, choice] of [
			[0, "background"],
			[1, "recede"],
			...
			assert(
				(await question.locator("input, textarea").count()) === 0,
				`question ${questionIndex + 1} offered a free-text field`,
			)
		...
		assert(cardCount > 0, "answering the questions narrowed the library to nothing")
		// A divider is the wrong object for a hero background, not a worse match.
		assert(
			(await recipes.locator('[data-generate-recipe="section-edge"]').count()) === 0,
			"a section divider was offered as a hero background",
		)
		...
		assert((origin?.answers as Record<string, string>)?.purpose === "background", "the answers were not recorded")
		...
		return `${cardCount} recipes narrowed from 2 clicked questions, ${variantCount} distinct variants, saved as @hero-wash with reproducible provenance, record readable behind the chip`
	})
	...
	await scenario("bh. the paid lane is offered honestly and refuses without credentials", async () => {
		...
		// The cutout is its own job in the interview, and its recipe is offered on
		// the same honest terms: shown whether or not this machine can pay for it.
		await chrome.getByTestId("assets-generate").click()
		await panel.waitFor({ timeout: 15_000 })
		for (const choice of ["cutout", "balanced"]) {
			const question = panel.getByTestId("generate-question")
			await question.waitFor({ timeout: 15_000 })
			await question.locator(`[data-generate-choice="${choice}"]`).click()
		}
		const cutouts = panel.getByTestId("generate-recipes")
		await cutouts.waitFor({ timeout: 20_000 })
		const cutoutCard = cutouts.locator('[data-generate-recipe="product-cutout"]')
		assert((await cutoutCard.count()) === 1, "the cutout purpose does not offer the cutout recipe")
		if (!configured) {
			assert(await cutoutCard.isDisabled(), "an unpayable cutout recipe was still clickable")
		}
		await panel.getByText("Cancel").click()

		return configured
			? "photograph and cutout recipes offered as runnable — credentials present in this environment"
			: `${marked} photograph recipe(s) plus the cutout shown, disabled, and explaining that the free lanes still work`
	})
	...
	if (!PAID) {
		...
		await scenario("bm. a cutout is generated on a key Caret chose, and lands with alpha", async () => {
			...
			const recipes = panel.getByTestId("generate-recipes")
			await recipes.waitFor({ timeout: 20_000 })
			await recipes.locator('[data-generate-recipe="product-cutout"]').click()
			...
			await chrome.getByTestId("assets-view").getByText("Done").click()
			return `${usable} keyed cutout(s) offered (${keyFailures} refused or unkeyable), saved as @cutout-object, ${entry?.mime} with alpha, cost recorded`
		})
	}
