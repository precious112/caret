...
footer: Caret CLI 2.0
date: January 2026
---

# NAME

caret - AI coding assistant in your terminal

# SYNOPSIS

**caret** [*prompt*] [*options*]

**caret** *command* [*options*] [*arguments*]

# DESCRIPTION

**caret** is a command-line interface for the Caret AI coding assistant. It provides the same powerful AI capabilities as the VS Code extension, directly in your terminal.

Caret is an autonomous AI agent that can read, write, and execute code across your projects. He can create and edit files, run terminal commands, use a headless browser, and more—all while asking for your approval before taking actions.
...
**Interactive Mode** :   When you run **caret** without arguments, it launches an interactive welcome prompt with a rich terminal UI. You can type your task, view conversation history, and interact with Caret in real-time.

**Task Mode** :   Run **caret "prompt"** or **caret task "prompt"** to immediately start a task. If stdin is a TTY, you'll see the interactive UI. If stdin is piped or output is redirected, the CLI automatically switches to plain text mode.
...
**ACT MODE** :   Caret actively uses tools to accomplish tasks. He can read files, write code, execute commands, use a headless browser, and more. This is the default mode for task execution.

**PLAN MODE** :   Caret gathers information and creates a detailed plan before implementation. He explores the codebase, asks clarifying questions, and presents a strategy for user approval before switching to ACT MODE.
...
**caret task** *prompt* [*options*]

**caret t** *prompt* [*options*] :   Create and run a new task. Options:
...
**\--config** *path* :   Path to Caret configuration directory
...
**caret history** [*options*]

**caret h** [*options*] :   Display previous tasks. Options:
...
**caret config** [*options*] :   Display global and workspace state. Options:
...
**caret auth** [*options*] :   Launch interactive authentication wizard, or use quick setup flags. Options:
...
**caret update** [*options*] :   Check npm for newer versions. Options:
...
**caret dev log** :   Open the log file for debugging.

# DEFAULT COMMAND OPTIONS

When running **caret** with just a prompt (no subcommand), these options are available:
...
# Run a task directly
caret "Create a hello world function in Python"

# Run with verbose output and extended thinking
caret -v --thinking "Analyze this codebase architecture"
```
...
caret -p "Design a REST API for user management"

# Run in act mode with auto-approval (yolo)
caret -y "Fix the typo in README.md"
```
...
caret -m claude-sonnet-4-5-20250929 "Refactor this function"

# Quick auth setup with model
caret auth -p anthropic -k sk-ant-xxxxx -m claude-sonnet-4-5-20250929
```
...
caret task -i screenshot.png diagram.jpg "Fix the UI based on these images"

# Or use inline image references in the prompt
caret "Fix the layout shown in @./screenshot.png"
```

## Piped Input

```bash
# Pipe file contents to Caret
cat README.md | caret "Summarize this document"

# Pipe with additional prompt
echo "function add(a, b) { return a + b }" | caret "Add TypeScript types to this"

# Combine piped input with a prompt
git diff | caret "Review these changes and suggest improvements"
```
...
caret --json "What files are in this directory?" | jq '.text'

# Yolo mode for automated workflows (auto-approves all actions), forces plain text output
caret -y "Run the test suite and fix any failures"
```
...
caret history -n 20 -p 2
```

## Resuming Tasks

```bash
# Resume a task by ID (get IDs from caret history)
caret -T abc123def

# Resume a task with a follow-up message
caret -T abc123def "Now add unit tests for the changes"

# Resume the most recent task from the current directory
caret --continue

# Resume in plan mode to review before continuing
caret -T abc123def -p "What's left to do?"

# Resume with yolo mode for automated continuation
caret -T abc123def -y "Continue with the implementation"
```
...
caret auth -p anthropic -k sk-ant-api-xxxxx

# Quick setup for OpenAI
caret auth -p openai-native -k sk-xxxxx -m gpt-4o

# OpenAI-compatible provider with custom base URL
caret auth -p openai -k your-api-key -b https://api.example.com/v1
```

# ENVIRONMENT

**CARET_DIR** :   Override the default configuration directory. When set, Caret stores all data in this directory instead of `~/.caret/data/`.

**CARET_COMMAND_PERMISSIONS** :   JSON configuration for restricting which shell commands Caret can execute. When set, commands are validated against allow/deny patternks before execution. When not set, all commands are allowed.
...
export CARET_COMMAND_PERMISSIONS='{"allow": ["npm *", "git *"]}'

# Allow development commands but deny dangerous ones. Deny not strictly required here since allow is set.
export CARET_COMMAND_PERMISSIONS='{"allow": ["npm *", "git *", "node *"], "deny": ["rm -rf *", "sudo *"]}'

# Allow file operations with redirects
export CARET_COMMAND_PERMISSIONS='{"allow": ["cat *", "echo *"], "allowRedirects": true}'
```
...
Caret is developed by Caret Bot Inc. and the open source community.
