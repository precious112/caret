...
- **The verify windows look stuck during model waits and the user WILL interact with them.** Three certification runs died at the same "mystery" moment before the cause was found: the user closing a seemingly-frozen test window (ff waits silently on a model for up to five minutes). Compounding it, `--user-data-dir` was silently ignored (Electron doesn't honour it; `desktop/main/index.ts` now applies it via `app.setPath` before the prefs/lock are touched) — so test runs shared the user's real profile: their preferences got overwritten by test settings, session-restore reopened temp fixtures in their own app, and the shared single-instance lock made user-opened and test instances kill each other. If a run dies with "window CLOSED" stamps in main.log during a long model wait, ask the user before diagnosing code.
