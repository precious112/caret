...
- **A permission REJECTION is terminal on the pinned server — the loop dies, the model is never consulted.** Measured 2026-08-26: `done` arrived 0.2s after a deny, far too fast for a model round trip, so the model never saw the rejection in-turn (the rejection text on the tool part is a record, not something any step read). The binary shows why: a reject FAILS the tool's deferred as a thrown error and cancels the session's other pending asks on the way down. Consequences: (1) the conversation's empty-close NUDGE is currently the ONLY channel through which a model ever learns a refusal happened; (2) the feedback Caret attaches to denies (`message` in the reply body) is doubly dead here — the pinned route drops the field AND the loop wouldn't continue to deliver it. Newer servers name reject-with-feedback `PermissionCorrectedError`, implying feed-back-and-continue. **At the pin bump, verify BOTH live: the feedback text lands in the tool result, and the loop continues past a rejection.** Do not re-diagnose a post-refusal dead turn as "the model gave up" — LLMs don't, and this one was never asked.
- **A failing provider stream is INVISIBLE on the pinned server's bus.** 1.18.11 retries a dead stream silently — its LLM runtime's onError only logs, and its /doc (which does list every event measured live, part.delta included) has no `session.retry.scheduled`. NEWER servers emit that event `{attempt, at, error}` and the client half bundled inside our own binary already consumes it — do not conclude "opencode doesn't expose this" from the pinned server alone. Caret maps the event now (armed, tested synthetically), and ON the pinned server gets the provider's words anyway by tailing `~/.local/share/opencode/log/opencode.log` for this session's `stream error` lines (`log-tail.ts`, verified end-to-end: a line appended mid-turn surfaced as a retry note). Delete the tailer at the pin bump. The silence clock complements (`lastEventAt` in ConversationState; the chat shows "nothing has arrived in Nm" past 60s of true quiet — thinking keeps the wire warm via reasoning deltas, so real quiet means a stalled or retrying provider). When bumping the pin, verify the retry event live and the note lights up for free.
