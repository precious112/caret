...
- [x] **Recipes read `foundation.json`.** A project on `deep-technical` gets a dark, cool,
...
- [x] Reuse the 6.5 interview surface — the same Caret-owned step machine and pick screens;
...
- [x] **Raster → Google Gemini image ("Nano Banana").** One adapter over the `@google/genai`
      ...
      backends.
- [x] **Decorative vector → code, not a model.** Seeded parametric generators for grainy and
...
- [ ] **Icons → curated open sets, installed.** *(deferred to Phase 7.5's install path — decided 2026-08-07)* Lucide, Phosphor, Radix, Heroicons. Generation
...
- [x] **Logos and marks → backend-authored SVG in a render-compare loop.** A Phase 6.4 session
      ...
      blank coordinates.
- [x] **3D objects → image-to-model through Tripo, with an LLM deciding the optimization.**
      *(Added 2026-08-07.)* A fifth lane: a single-object source image — generated by the
      raster lane's object-study recipe or picked from the library — goes through Tripo's
      image_to_model, and the chosen LLM decides face limit and texture size inside a bounded
      schema so the result doesn't weigh the page down (certified live: a 14MB draft became a
      528KB glb). A vision check refuses multi-object and scene sources **before** credits are
