...
| Originkit | **INSTALLS — WITH AN ACCOUNT KEY.** The CLI is real and the best-designed on this table (`list` returned 193 components unauthenticated; clean install layout, `--dry-run`/`--no-deps`/`--path` flags), but `add` requires sign-in: browser OAuth interactively, `ORIGINKIT_API_KEY` headlessly (verified by running `add tornado` — clear refusal naming both paths). Deliveries are quota'd even signed in: components 10/day, 30/week, shared across CLI/MCP/site copies | — (nothing can land without a key) | Not "uninstallable" — auth-gated: for Caret's unattended agent installs it needs a configured API key + quota budget. A free account + key unlocks a real install and a specimen |
