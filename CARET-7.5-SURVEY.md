...
| Originkit | **AUTH-GATED** — CLI is real and polished (`list` returned 193 components unauthenticated) but `add --dry-run` already refuses without an account API key; quotas apply after sign-in | — | Researched 2026-08-10 at the user's request. No public repo, components' licence undeclared, Framer-first source. An API key would unlock a real install + specimen |
